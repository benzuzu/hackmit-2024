...
export const generateAndStoreChapter = action({
  ...
  handler: async (ctx, {storyId}) => {
    ...
    const chapter: Chapter = await ctx.runAction(internal.chapter.storeGeneratedChapter, {storyId, generatedText: texts, generatedImageUrls: images})
    console.log("stored")
    return chapter
  }
})
  ...
  handler: async (_, args) => {
    try {
      const imagePromises = args.texts.map((text, index) => {
        const prompt = `Context: ${args.character1} ${args.character2}. Now, draw the following paragraph. Use the style of Une64e2g1l7URd6C : ${text}.`
